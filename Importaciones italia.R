# =========================================================
# TFG - CAPÍTULO 4
# SERIE 4.6: IMPORTACIONES ITALIA-ESPAÑA
# =========================================================
...
# =========================================================
df <- importaciones_italia
df$FECHA <- as.Date(df$FECHA)
df$euros <- as.numeric(df$euros)
df <- df[order(df$FECHA), ]
df <- na.omit(df)

y0 <- ts(df$euros, start = c(2000, 1), frequency = 12)

cat("\n=====================================================\n")
cat("4.X.1 DESCRIPCIÓN DE LA SERIE: IMPORTACIONES ITALIA-ESPAÑA\n")
cat("=====================================================\n")
...
plot(
  y0,
  main = "Importaciones Italia-España - Serie original",
  ylab = "Euros",
  col = "black",
  lwd = 2
)
...
plot(
  y,
  main = "Importaciones Italia-España - Serie en logaritmos",
  ylab = "log(euros)",
  col = "blue",
  lwd = 2
)
...
forecast::ggseasonplot(y0, year.labels = TRUE, year.labels.left = TRUE) +
  ylab("Euros") +
  ggtitle("Importaciones Italia-España - Seasonal plot (serie original)")

forecast::ggseasonplot(y, year.labels = TRUE, year.labels.left = TRUE) +
  ylab("log(euros)") +
  ggtitle("Importaciones Italia-España - Seasonal plot (log)")

forecast::ggsubseriesplot(y0) +
  ylab("Euros") +
  ggtitle("Importaciones Italia-España - Subseries plot")

acf(y, main = "ACF de log(importaciones)")
pacf(y, main = "PACF de log(importaciones)")

dy  <- diff(y, differences = 1)
dsy <- diff(y, lag = frequency(y), differences = 1)

plot(dy,  main = "Primera diferencia de log(importaciones)", ylab = "Diferencia")
plot(dsy, main = "Diferencia estacional de log(importaciones)", ylab = "Diferencia estacional")
...
p_classic <- ggplot(ranking_classic, aes(x = Model, y = RMSE)) +
  ...
  geom_text(aes(label = round(RMSE, 4)),
            ...
            size = 4) +
  labs(
    title = "Comparación de modelos clásicos - Importaciones Italia-España (Test)",
    x = "Modelo",
    y = "RMSE"
  ) +
...
p_sarima <- ggplot(df_plot_sarima, aes(x = time)) +
  ...
  labs(
    title = "Ajuste del modelo SARIMA sobre importaciones (log)",
    x = "Tiempo",
    y = "log(euros)",
    colour = ""
  ) +
...
models_import <- models_classic
models_import[[best_dl_name]] <- best_dl_fc

ranking_final <- rank_models_test(models_import, y_test)
...
p_final <- ggplot(ranking_final, aes(x = reorder(Model, RMSE), y = RMSE)) +
  ...
  geom_text(aes(label = round(RMSE, 4)),
            ...
            size = 4) +
  labs(
    title = "Comparación final de modelos - Importaciones Italia-España (Test)",
    x = "Modelo",
    y = "RMSE"
  ) +
  theme_minimal(base_size = 13) +
...
ts.plot(
  ...
  lwd = 2,
  ylab = "log(euros)",
  main = "Importaciones Italia-España: Serie real y predicciones finales"
)
...
p_dl_import <- ggplot() +
  ...
  geom_line(data = df_pred,
            aes(x = time, y = value, colour = "GRU"),
            linewidth = 1) +
  scale_colour_manual(
    values = c("GRU" = "blue", "Serie real" = "black")
  ) +
  labs(
    title = "Predicción del mejor modelo GRU sobre log(importaciones)",
    x = "Tiempo",
    y = "log(euros)",
    colour = ""
  ) +
...
print(p_dl_import)
# =========================================================
# 7) 4.X.6 ANÁLISIS DE ESTABILIDAD DEL MODELO DL
# =========================================================
...
p_hybrid <- ggplot(ranking_hybrid, aes(x = Model, y = RMSE)) +
  ...
  geom_text(aes(label = round(RMSE, 4)),
            ...
            size = 4) +
  labs(
    title = "SARIMA vs SARIMA + LSTM híbrido - Importaciones Italia-España",
    x = "Modelo",
    y = "RMSE"
  ) +
  theme_minimal(base_size = 13) +
...
ts.plot(
  ...
  lwd = 2,
  ylab = "log(euros)",
  main = "Importaciones Italia-España: SARIMA vs SARIMA+LSTM híbrido"
)
...
# Importaciones Italia-España: SARIMA vs SARIMA + LSTM híbrido
# =========================================================
...
ts.plot(
  ...
  ylab = "log(euros)",
  xlab = "Año",
  main = "Importaciones Italia-España: SARIMA vs SARIMA+LSTM híbrido (zoom últimos 4 años)",
  ylim = ylim_zoom
)
...
ranking_final_plot$Grupo[ranking_final_plot$Model == "STL_ETS"] <- "Clásico (Mejor)"
...
ranking_all$Grupo[ranking_all$Model == "STL_ETS"] <- "Clásico (Mejor)"
...
p_all <- ggplot(ranking_all, aes(x = Model, y = RMSE, fill = Grupo)) +
  ...
  labs(
    title = "Comparación final completa de modelos - Importaciones Italia-España",
    x = "Modelo",
    y = "RMSE",
    fill = ""
  ) +

...
# h = 1 como aproximación práctica homogénea
# -----------------------------
...
# =========================================================
# 3) 4.1.1 DESCRIPCIÓN DE LA SERIE
# =========================================================
y0 <- AirPassengers

cat("\n=====================================================\n")
cat("4.1.1 DESCRIPCIÓN DE LA SERIE: AIRPASSENGERS\n")
cat("=====================================================\n")
...
# =========================================================
# 4) 4.1.2 ANÁLISIS EXPLORATORIO Y TRANSFORMACIONES
# =========================================================

plot(
  y0,
  main = "AirPassengers - Serie original",
...
plot(
  y,
  main = "AirPassengers - Serie en logaritmos",
  ylab = "log(Pasajeros)",
  col = "blue",
  lwd = 2
)
...
forecast::ggseasonplot(y0, year.labels = TRUE, year.labels.left = TRUE) +
  ylab("Pasajeros") +
  ggtitle("AirPassengers - Seasonal plot (serie original)")

forecast::ggseasonplot(y, year.labels = TRUE, year.labels.left = TRUE) +
  ylab("log(Pasajeros)") +
  ggtitle("AirPassengers - Seasonal plot (log)")

forecast::ggsubseriesplot(y0) +
  ylab("Pasajeros") +
  ggtitle("AirPassengers - Subseries plot")

acf(y, main = "ACF de log(AirPassengers)")
pacf(y, main = "PACF de log(AirPassengers)")

dy  <- diff(y, differences = 1)
dsy <- diff(y, lag = frequency(y), differences = 1)

plot(dy, main = "Primera diferencia de log(AirPassengers)", ylab = "Diferencia")
plot(dsy, main = "Diferencia estacional de log(AirPassengers)", ylab = "Diferencia estacional")

acf(dy, main = "ACF de la primera diferencia")
acf(dsy, main = "ACF de la diferencia estacional")

# =========================================================
# 5) 4.1.3 MODELIZACIÓN CLÁSICA
# =========================================================
h <- 12
n <- length(y)
...
fit_sarima <- Arima(y_train, order = c(0,1,1), seasonal = c(0,1,1))
fc_sarima  <- forecast(fit_sarima, h = h)

fit_auto <- auto.arima(y_train, seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
...
cat("\n================ LJUNG-BOX SARIMA ================\n")
print(Box.test(res_sarima, lag = 24, type = "Ljung-Box"))
...
# -----------------------------
# 5.5 Gráfico del ranking clásico
# -----------------------------
...
p_classic <- ggplot(ranking_classic, aes(x = Model, y = RMSE)) +
  ...
  geom_text(aes(label = round(RMSE, 4)),
            ...
            size = 4) +
  labs(
    title = "Comparación de modelos clásicos - AirPassengers (Test)",
    x = "Modelo",
    y = "RMSE"
  ) +
...
# -----------------------------
# 5.6 Gráfico del SARIMA ajustado
# -----------------------------
...
p_sarima <- ggplot(df_plot_sarima, aes(x = time)) +
  ...
  labs(
    title = "Ajuste del modelo SARIMA sobre AirPassengers (log)",
    x = "Tiempo",
    y = "log(Pasajeros)",
    colour = ""
  ) +
...
# =========================================================
# 6) 4.1.4 MODELIZACIÓN MEDIANTE DEEP LEARNING
# =========================================================
grid <- expand.grid(
  type = c("LSTM", "GRU"),
  lookback = c(12, 24),
...
models_air <- models_classic
models_air[[best_dl_name]] <- best_dl_fc

ranking_final <- rank_models_test(models_air, y_test)

cat("\n================ RANKING FINAL EN TEST =================\n")
print(ranking_final)

# -----------------------------
# 6.3 Gráfico del mejor DL
# -----------------------------
...
p_dl <- ggplot() +
  ...
  geom_line(data = df_plot_dl_pred,
            aes(x = time, y = Pred_DL, colour = "LSTM"),
            linewidth = 1) +
  scale_colour_manual(
    values = c("Serie real" = "black", "LSTM" = "blue")
  ) +
  labs(
    title = "Predicción del mejor modelo DL sobre AirPassengers (log)",
    x = "Tiempo",
    y = "log(Pasajeros)",
    colour = ""
  ) +
...
# =========================================================
# 7) 4.1.5 COMPARACIÓN DE RESULTADOS PREDICTIVOS
# =========================================================
cat("\n=====================================================\n")
...
p_final <- ggplot(ranking_final, aes(x = Model, y = RMSE, fill = Tipo)) +
  ...
  labs(
    title = "Comparación final de modelos - AirPassengers (Test)",
    x = "Modelo",
    y = "RMSE",
    fill = ""
  ) +
...
# =========================================================
# 8) 4.1.6 ANÁLISIS DE ESTABILIDAD DEL MEJOR MODELO DL
# =========================================================
...
ts.plot(
  ...
  lwd = 2,
  ylab = "log(Passengers)",
  main = paste0("AirPassengers - Repeticiones del mejor ", best_type)
)
...
ranking_hybrid$Tipo <- ifelse(
  ...
  "Clásico (Mejor)",
  ifelse(
    ranking_hybrid$Model == "LSTM_BEST",
    "Deep Learning",
    ifelse(
...
p_hybrid <- ggplot(ranking_hybrid, aes(x = Model, y = RMSE, fill = Tipo)) +
  ...
  labs(
    title = "Comparación final de modelos (incluyendo híbrido) - AirPassengers",
    x = "Modelo",
    y = "RMSE",
    fill = ""
  ) +
...
ts.plot(
  ...
  lwd = 2,
  ylab = "log(Passengers)",
  main = "AirPassengers: SARIMA vs SARIMA+LSTM híbrido"
)
...
# =========================================================
write.csv(ranking_classic, "airpassengers_ranking_classic.csv", row.names = FALSE)
write.csv(results_df, "airpassengers_dl_grid_results.csv", row.names = FALSE)
write.csv(ranking_final, "airpassengers_final_ranking.csv", row.names = FALSE)
write.csv(rep_results, "airpassengers_lstm_repeticiones.csv", row.names = FALSE)
write.csv(summary_rep, "airpassengers_lstm_repeticiones_resumen.csv", row.names = FALSE)
write.csv(comparison_hybrid, "airpassengers_sarima_vs_hybrid.csv", row.names = FALSE)
write.csv(ranking_hybrid, "airpassengers_hybrid_ranking.csv", row.names = FALSE)

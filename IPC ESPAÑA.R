...
# =========================================================
# 3) 4.X.1 DESCRIPCIÓN DE LA SERIE
# =========================================================
df <- IPC_ESPAÑA
...
cat("\n=====================================================\n")
cat("4.X.1 DESCRIPCIÓN DE LA SERIE: IPC ESPAÑA\n")
cat("=====================================================\n")
...
# =========================================================
# 4) 4.X.2 ANÁLISIS EXPLORATORIO Y TRANSFORMACIONES
# =========================================================
plot(
  y0,
  main = "IPC España - Serie original",
  ylab = "Tasa",
  col = "black",
  lwd = 2
)
...
# IMPORTANTE:
# No se aplica transformación logarítmica porque la serie está expresada en tasas.
# Además, al tratarse de una serie de inflación mensual en tasas, la modelización
# se plantea directamente sobre la serie original.
y <- y0

plot(
  y,
  main = "IPC España - Serie utilizada en modelización",
  ylab = "Tasa",
  col = "blue",
  lwd = 2
)
...
forecast::ggseasonplot(y0, year.labels = TRUE, year.labels.left = TRUE) +
  ylab("Tasa") +
  ggtitle("IPC España - Seasonal plot")

forecast::ggsubseriesplot(y0) +
  ylab("Tasa") +
  ggtitle("IPC España - Subseries plot")

acf(y, main = "ACF del IPC España")
pacf(y, main = "PACF del IPC España")

dy  <- diff(y, differences = 1)
dsy <- diff(y, lag = frequency(y), differences = 1)

plot(dy,  main = "Primera diferencia del IPC España", ylab = "Diferencia")
plot(dsy, main = "Diferencia estacional del IPC España", ylab = "Diferencia estacional")

acf(dy,  main = "ACF de la primera diferencia")
acf(dsy, main = "ACF de la diferencia estacional")

# =========================================================
# 5) 4.X.3 MODELIZACIÓN CLÁSICA
# =========================================================
h <- 12
n <- length(y)
...
# AutoARIMA como benchmark automático
fit_auto <- auto.arima(
  y_train,
  seasonal = TRUE,
  stepwise = FALSE,
...
cat("AIC SARIMA manual:", AIC(fit_sarima), "\n")
...
p_classic <- ggplot(ranking_classic, aes(x = Model, y = RMSE)) +
  ...
  geom_text(aes(label = round(RMSE, 4)),
            ...
            size = 4) +
  labs(
    title = "Comparación de modelos clásicos - IPC España (Test)",
    x = "Modelo",
    y = "RMSE"
  ) +
...
p_sarima <- ggplot(df_plot_sarima, aes(x = time)) +
  ...
  labs(
    title = "Ajuste del modelo SARIMA sobre IPC España",
    x = "Tiempo",
    y = "Tasa",
    colour = ""
  ) +
...
# =========================================================
# 6) 4.X.4 MODELIZACIÓN MEDIANTE DEEP LEARNING
# =========================================================
grid <- expand.grid(
  type = c("LSTM", "GRU"),
  lookback = c(12, 24, 36),
...
models_ipc <- models_classic
models_ipc[[best_dl_name]] <- best_dl_fc

ranking_final <- rank_models_test(models_ipc, y_test)
...
p_final <- ggplot(ranking_final, aes(x = reorder(Model, RMSE), y = RMSE)) +
  ...
  geom_text(aes(label = round(RMSE, 4)),
            ...
            size = 4) +
  labs(
    title = "Comparación final de modelos - IPC España (Test)",
    x = "Modelo",
    y = "RMSE"
  ) +
  theme_minimal(base_size = 13) +
...
ts.plot(
  ...
  lwd = 2,
  ylab = "Tasa",
  main = "IPC España: Serie real y predicciones finales"
)
...
p_dl_ipc <- ggplot() +
  ...
  geom_line(data = df_pred,
            aes(x = time, y = value, colour = as.character(best_row$type)),
            linewidth = 1) +
  scale_colour_manual(
    values = c("Serie real" = "black", "LSTM" = "blue", "GRU" = "blue")
  ) +
  labs(
    title = paste("Predicción del mejor modelo", as.character(best_row$type), "sobre IPC España"),
    x = "Tiempo",
    y = "Tasa",
    colour = ""
  ) +
...
# IPC España: mejor modelo Deep Learning
# =========================================================

# Número de observaciones a mostrar: 4 años mensuales = 48 meses
n_zoom <- 4 * frequency(y)

# Tiempo inicial del zoom
start_zoom <- as.numeric(time(y))[max(1, length(y) - n_zoom + 1)]

# Data frame de la serie real (solo últimos 4 años)
df_real <- data.frame(
  time = as.numeric(time(y)),
  value = as.numeric(y)
)

df_real_zoom <- df_real[df_real$time >= start_zoom, ]

# Data frame de predicciones (solo las que caen en la ventana de zoom)
df_pred <- data.frame(
  time = as.numeric(time(y_test)),
  value = as.numeric(best_dl_fc$mean)
)

df_pred_zoom <- df_pred[df_pred$time >= start_zoom, ]
...
p_dl_ipc <- ggplot() +
  ...
  labs(
    title = paste(
      ...
      as.character(best_row$type),
      "sobre IPC España (zoom últimos 4 años)"
    ),
    x = "Tiempo",
    y = "Tasa",
    colour = ""
...
print(p_dl_ipc)
# =========================================================
# 7) 4.X.5 ANÁLISIS DE ESTABILIDAD DEL MODELO DL
# =========================================================
...
p_hybrid <- ggplot(ranking_hybrid, aes(x = Model, y = RMSE)) +
  ...
  geom_text(aes(label = round(RMSE, 4)),
            ...
            size = 4) +
  labs(
    title = "SARIMA vs SARIMA + LSTM híbrido - IPC España",
    x = "Modelo",
    y = "RMSE"
  ) +
  theme_minimal(base_size = 13) +
...
ts.plot(
  ...
  lwd = 2,
  ylab = "Tasa",
  main = "IPC España: SARIMA vs SARIMA+LSTM híbrido"
)
...
# =========================================================
# GRÁFICO ZOOM: ÚLTIMOS 4 AÑOS
# IPC España: SARIMA vs SARIMA + LSTM híbrido
# =========================================================
...
ts.plot(
  ...
  ylab = "Tasa",
  xlab = "Año",
  main = "IPC España: SARIMA vs SARIMA+LSTM híbrido (zoom últimos 4 años)",
  ylim = ylim_zoom
)
...
# =========================
# Gráfico final de modelos (incluyendo híbrido)
# =========================
ranking_all <- rbind(
  ranking_final,
  data.frame(
...
p_all <- ggplot(ranking_all, aes(x = Model, y = RMSE, fill = Grupo)) +
  ...
  labs(
    title = "Comparación final de modelos - IPC España (incluyendo híbrido)",
    x = "Modelo",
    y = "RMSE",
    fill = ""
  ) +
